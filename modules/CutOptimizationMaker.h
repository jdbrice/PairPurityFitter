...
public:
	...
	virtual void make(){
		...
		for ( size_t i = 0 ;i < bins["mass"].nBins(); i++ ){
			...
			string n = string(TString::Format( "template_%s_%s_m%lu", sign.c_str(), "pipi", i ));
			...
			hpurity_sig->Draw();
			hpurity_bg->Draw("same");
			hpurity_sigbg->Draw("same");
			...
			TLegend * leg = new TLegend( 0.2, 0.78, 0.6, 0.95 );
			leg->SetNColumns( 2 );
			leg->SetBorderSize( 0 );
			leg->AddEntry( hpurity_sig, "signal purity" );
			leg->AddEntry( hpurity_bg, "background purity" );
			leg->AddEntry( hpurity_sigbg, "S / B" );
			leg->AddEntry( hsignificance, "S / sqrt( S + B )" );
			leg->AddEntry( hmaxsignificance, "Max( S / sqrt( S + B ) )", "p" );
			leg->Draw();
